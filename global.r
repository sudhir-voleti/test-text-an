...
{ #require("tm")
  ...
  x  =  tolower(x)
  if(html_tags){ x  =  gsub("<.*?>", " ", x) }# regex for removing HTML tags
  if(numbers){ x  =  removeNumbers(x) } # convert to lower case characters
...
distill.cog.tcm = function(mat1, # input TCM or DTM MAT
  ...
  require(igraph)
  mat1 = as.matrix(mat1)
  if (mattype == "DTM"){ mat1 = tcrossprod(t(mat1)) }
